...
class TestPipDetection:
    ...
    def test_pip3_detected_successfully(self, detector):
        ...
        with patch('shutil.which', side_effect=lambda cmd: '/usr/bin/pip3' if cmd == 'pip3' else None):
            with patch('subprocess.run', return_value=mock_result):
    ...
    def test_pip_detected_when_pip3_missing(self, detector):
        ...
        with patch('shutil.which', side_effect=lambda cmd: '/usr/bin/pip' if cmd == 'pip' else None):
            with patch('subprocess.run', return_value=mock_result):
                status = detector.detect_pip()
...
class TestDetectMirrorTools:
    ...
    def test_detect_mirror_tools_returns_all(self, detector):
        ...
        with patch.object(detector, 'detect_node') as mock_node, \
             patch.object(detector, 'detect_bun') as mock_bun, \
