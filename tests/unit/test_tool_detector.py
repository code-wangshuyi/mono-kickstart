...
class TestCommandAvailability:
    ...
    def test_command_availability_with_various_commands(self, detector):
        ...
        commands = ["node", "npm", "bun", "uv", "conda", "claude", "codex", "opencode"]
...
class TestOpenCodeDetection:
    """Test OpenCode CLI detection logic"""

    def test_opencode_detected_successfully(self, detector):
        """Test OpenCode CLI detection when installed"""
        mock_result = Mock()
        mock_result.stdout = "opencode 1.1.0\n"
        mock_result.stderr = ""

        with patch("shutil.which", return_value="/usr/local/bin/opencode"):
            with patch("subprocess.run", return_value=mock_result):
                status = detector.detect_opencode()

                assert status.name == "opencode"
                assert status.installed is True
                assert status.version == "1.1.0"
                assert status.path == "/usr/local/bin/opencode"

    def test_opencode_not_detected_when_missing(self, detector):
        """Test OpenCode CLI detection when not installed"""
        with patch("shutil.which", return_value=None):
            status = detector.detect_opencode()
...
class TestDetectAllTools:
    ...
    def test_detect_all_tools_returns_all_tools(self, detector):
        ...
        with (
            patch.object(detector, "detect_nvm") as mock_nvm,
            ...
            patch.object(detector, "detect_opencode") as mock_opencode,
            ...
            mock_opencode.return_value = ToolStatus("opencode", False)
    ...
    def test_detect_all_tools_with_mixed_installation_states(self, detector):
        ...
        with (
            patch.object(detector, "detect_nvm", return_value=ToolStatus("nvm", True, "0.40.4")),
            ...
            patch.object(
                detector, "detect_claude_code", return_value=ToolStatus("claude-code", False)
            ),
            patch.object(detector, "detect_codex", return_value=ToolStatus("codex", False)),
            patch.object(detector, "detect_opencode", return_value=ToolStatus("opencode", False)),
            ...
            patch.object(
                detector, "detect_spec_kit", return_value=ToolStatus("spec-kit", True, "1.5.0")
            ),
            ...
            assert (
                not_installed_count == 8
            )  # conda, bun, gh, claude-code, codex, opencode, uipro, bmad-method


class TestPipDetection:
    """Test pip detection logic"""

    def test_pip3_detected_successfully(self, detector):
        """Test pip detection when pip3 is available"""
        mock_result = Mock()
        mock_result.stdout = "pip 23.0.1\n"
        mock_result.stderr = ""

        with patch(
            "shutil.which", side_effect=lambda cmd: "/usr/bin/pip3" if cmd == "pip3" else None
        ):
            with patch("subprocess.run", return_value=mock_result):
                status = detector.detect_pip()

                assert status.name == "pip"
                assert status.installed is True
                assert status.version == "23.0.1"
                assert status.path == "/usr/bin/pip3"

    def test_pip_detected_when_pip3_missing(self, detector):
        """Test pip detection falls back to pip when pip3 is missing"""
        mock_result = Mock()
        mock_result.stdout = "pip 22.0.0\n"
        mock_result.stderr = ""

        with patch(
            "shutil.which", side_effect=lambda cmd: "/usr/bin/pip" if cmd == "pip" else None
        ):
            with patch("subprocess.run", return_value=mock_result):
                status = detector.detect_pip()
                ...
                assert status.name == "pip"
                assert status.installed is True
                assert status.version == "22.0.0"
                assert status.path == "/usr/bin/pip"

    def test_pip_not_detected_when_both_missing(self, detector):
        """Test pip detection when neither pip3 nor pip are available"""
        with patch("shutil.which", return_value=None):
            status = detector.detect_pip()
...
class TestDetectMirrorTools:
    """Test detect_mirror_tools method"""

    def test_detect_mirror_tools_returns_all(self, detector):
        """Test detect_mirror_tools returns all 5 mirror tools"""
        with (
            patch.object(detector, "detect_node") as mock_node,
            patch.object(detector, "detect_bun") as mock_bun,
            patch.object(detector, "detect_pip") as mock_pip,
            patch.object(detector, "detect_uv") as mock_uv,
            patch.object(detector, "detect_conda") as mock_conda,
        ):
            mock_node.return_value = ToolStatus("node", True, "20.11.0", "/usr/bin/node")
            mock_bun.return_value = ToolStatus("bun", False)
            mock_pip.return_value = ToolStatus("pip", True, "23.0.1", "/usr/bin/pip3")
            mock_uv.return_value = ToolStatus("uv", True, "0.1.5", "/usr/local/bin/uv")
            mock_conda.return_value = ToolStatus("conda", False)

            result = detector.detect_mirror_tools()
    ...
    def test_detect_mirror_tools_uses_detect_node_for_npm(self, detector):
        """Test detect_mirror_tools uses detect_node for npm key"""
        with (
            patch.object(detector, "detect_node") as mock_node,
            patch.object(detector, "detect_bun", return_value=ToolStatus("bun", False)),
            patch.object(detector, "detect_pip", return_value=ToolStatus("pip", False)),
            patch.object(detector, "detect_uv", return_value=ToolStatus("uv", False)),
            patch.object(detector, "detect_conda", return_value=ToolStatus("conda", False)),
        ):
            mock_node.return_value = ToolStatus("node", True, "20.11.0", "/usr/bin/node")
            ...
            # Verify detect_node was called
            mock_node.assert_called_once()

            # Verify npm key uses node status
            assert result["npm"].name == "node"
            assert result["npm"].installed is True
            assert result["npm"].version == "20.11.0"
...
class TestToolStatusDataClass:
    ...
    def test_tool_status_creation_with_all_fields(self):
        ...
        status = ToolStatus(name="node", installed=True, version="20.11.0", path="/usr/bin/node")
