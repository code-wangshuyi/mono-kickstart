...
class TestConfigureMirrors:
    ...
    def test_configure_mirrors_success(self, orchestrator):
        """测试成功配置所有镜像源"""
        with patch.object(orchestrator.mirror_configurator, 'configure_all', return_value={
            "npm": True,
            "bun": True,
            "uv": True
        }):
...
class TestRunInit:
    ...
    def test_run_init_success(self, orchestrator):
        ...
        # Mock 镜像源配置
        with patch.object(orchestrator, '_create_installer', side_effect=lambda name: create_mock_installer(name)), \
             patch.object(orchestrator.mirror_configurator, 'configure_npm_mirror', return_value=True), \
             patch.object(orchestrator.mirror_configurator, 'configure_bun_mirror', return_value=True), \
             patch.object(orchestrator.mirror_configurator, 'configure_uv_mirror', return_value=True), \
    ...
    def test_run_init_only_configures_mirrors_for_successful_tools(self, orchestrator):
        ...
        with patch.object(orchestrator, '_create_installer', side_effect=lambda name: create_mock_installer(name)), \
             patch.object(orchestrator.mirror_configurator, 'configure_npm_mirror', return_value=True) as mock_npm, \
             patch.object(orchestrator.mirror_configurator, 'configure_bun_mirror', return_value=True) as mock_bun, \
             patch.object(orchestrator, 'create_project', return_value=(True, None)):
...
class TestRunUpgrade:
    ...
    def test_run_upgrade_all_tools(self, orchestrator):
        ...
        with patch.object(orchestrator.tool_detector, 'detect_all_tools', return_value=mock_tool_status), \
             patch.object(orchestrator, '_create_installer', side_effect=lambda name: create_mock_installer(name)):
...
class TestCreateInstaller:
    ...
    def test_create_installer_uses_config(self, orchestrator):
        """测试安装器使用配置"""
        orchestrator.config.tools["nvm"] = ToolConfig(
            enabled=True,
            version="0.40.4"
        )
