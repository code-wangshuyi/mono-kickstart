...
def create_mock_orchestrator():
    ...
    mock_orchestrator.install_tool.return_value = InstallReport(
        tool_name="test", result=InstallResult.SUCCESS, message="Test success"
    )
...
class TestCommandEntryEquivalence:
    ...
    **Validates: Requirements 7.8, 10.8**
        ...
        with patch("sys.argv", ["mk"] + command_args):
            with patch("sys.stdout", new=StringIO()) as mk_stdout:
                with patch("sys.stderr", new=StringIO()) as mk_stderr:
                    with patch(
                        "mono_kickstart.orchestrator.InstallOrchestrator", return_value=mock_orch
                    ):
                        with patch(
                            "mono_kickstart.platform_detector.PlatformDetector"
                        ) as mock_detector:
                            # Mock 平台检测为支持的平台
                            ...
                            with patch("mono_kickstart.config.ConfigManager") as mock_config:
                                ...
                                mock_config.return_value.load_with_priority.return_value = (
                                    mock_config_obj
                                )
                                mock_config.return_value.load_from_defaults.return_value = (
                                    mock_config_obj
                                )

                                with patch(
                                    "mono_kickstart.tool_detector.ToolDetector"
                                ) as mock_tool_detector:
                                    # Mock 工具检测器
    ...
    @given(subcommand=st.sampled_from(["init", "upgrade", "install"]))
    @settings(
        max_examples=10, suppress_health_check=[HealthCheck.function_scoped_fixture], deadline=None
    )
